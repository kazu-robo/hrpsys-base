                                 ...
                                 int length)
{
    ...
    bool same_side=true;
    while(same_side){
        opposite_initial_fs_index++;
        if(fs_side[0] != fs_side[opposite_initial_fs_index]){
            same_side=false;
        }
    }
    for(int i=length-1;i>=0;i--){
        if(i==0){
            dp[i]=footsteps_pos[i];
        }
        else if(i<opposite_initial_fs_index){ // 0歩目と1歩目が同じなど、逆足を動かすまで
            dp[i]=footsteps_pos[i] - footsteps_pos[i-1];
        }
            ...
            for(int j=i-1;j>=0;j--){
                if(fs_side[j]==0){
                    dp[i]=footsteps_pos[i] - footsteps_pos[j];
                    break;
                }
            }
        }
    ...
    setConstraintsList(std::move(new_constraints)); // motion
    setRefZMPList(loop);
    ...
    )
{
    ...
    // TODO
    // 1. ベタ書きでfootstepに従って動けるように done
    // 2. 入力とつなぐdone
    // 3. jump height とかの指定について
    ...
    const double take_off_z_vel = std::sqrt(2 * g_acc * default_jump_height); // todo jump_height
...
bool GaitGenerator::startRunning(const double dt, const double g_acc)
{
    ...
    for (const auto& constraint : constraints_list[cur_const_idx].constraints) {
        org_targets.push_back(constraint.targetCoord());
    } // constraints_list[cur_const_idx].constraintsがなんたるか
    // std::swap(jump_idx[0], land_idx[0]);
